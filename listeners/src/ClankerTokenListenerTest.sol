...
contract ClankerTokenListenerTest is ClankerTokenV4_0$OnTransferEvent {
    ...
    modifier onlyRetaketvToken(EventContext memory ctx) {
        ...
        if (!containsStreammDeployment(tokenContext)) revert NotARetaketvToken();
        _;
    }
    ...
    function onTransferEvent(
        ...
        uint8 factoryVersion = 0;

        if (deployedContract == CLANKER_V4_0_0_BASE) {
            factoryVersion = 4;
        }
        
        if (deployedContract == CLANKER_V3_1_FACTORY_BASE) {
            factoryVersion = 3;
        }
